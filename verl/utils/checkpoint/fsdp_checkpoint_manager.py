    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            if hasattr(model_config, "auto_map"):
                ...
                # 手动复制所有自定义 Python 文件
                # custom_object_save 可能不会保存所有需要的文件（如 modeling_mimo.py, configuration_mimo.py）
                if hasattr(model_config, "name_or_path") and model_config.name_or_path:
                    import shutil
                    import glob
                    
                    original_model_path = model_config.name_or_path
                    # 确保是本地路径且存在
                    if os.path.exists(original_model_path) and os.path.isdir(original_model_path):
                        # 需要复制的文件模式
                        ...
                        for pattern in custom_file_patterns:
                            for file_path in glob.glob(os.path.join(original_model_path, pattern)):
                                file_name = os.path.basename(file_path)
                                dest_path = os.path.join(hf_config_tokenizer_path, file_name)
                                # 只复制不存在的文件，避免覆盖 custom_object_save 已保存的文件
                                if not os.path.exists(dest_path):
                                    try:
                                        ...
                                        log_with_rank(
                                            f"Warning: Failed to copy {file_name} from {original_model_path}: {e}",
                                            rank=self.rank,
                                            logger=logger,
                                            log_only_rank_0=True,
                                        )
                        ...
                        if copied_files:
                            log_with_rank(
                                f"Copied custom code files to {hf_config_tokenizer_path}: {', '.join(copied_files)}",
                                rank=self.rank,
                                logger=logger,
                                log_only_rank_0=True,
                            )
        ...
        if self.should_save_hf_model:
            ...
            if self.rank == 0:
                ...
                # 根据模型配置是否有 auto_map 决定是否使用 trust_remote_code
                use_trust_remote_code = (
                    hasattr(model_config, "auto_map") 
                    and model_config.auto_map is not None
                    and len(model_config.auto_map) > 0
                )

                # 如果使用 trust_remote_code，需要确保从正确路径加载自定义代码
                # 因为 custom_object_save 已经将自定义代码保存到了新的检查点路径
                original_name_or_path = None
                if use_trust_remote_code and hasattr(model_config, "name_or_path") and model_config.name_or_path:
                    original_name_or_path = model_config.name_or_path
                    # 临时修改 name_or_path 指向新保存的检查点路径
                    # 这样 Transformers 可以从这里加载自定义代码文件（如 _fully_shard.py）
                    model_config.name_or_path = hf_local_path
                ...
                with init_empty_weights():
                        try:
                            ...
                            # 恢复原始的 name_or_path（如果修改过）
                            if original_name_or_path is not None:
